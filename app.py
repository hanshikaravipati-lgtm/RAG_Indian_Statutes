...
"""
    ...
    # ── 3. RAGAS Evaluation ───────────────────────────────────────────────────
    st.markdown("##RAG Evaluation — RAG'S Metrics")
    st.caption(
        f"Evaluated using **RAG'S framework** · "
        f"Judge LLM: `{JUDGE_MODEL}` (different from answer model)"
    )

    with st.spinner("Running RAG'S evaluation..."):
        try:
            ...
            with st.expander("ℹWhat do these metrics mean?"):
                st.markdown(f"""
...
> RAGAS uses `{JUDGE_MODEL}` internally to compute them.
            ...
            if not ground_truth_input.strip() and use_recall:
                st.warning(
                    "Context Recall is enabled but no ground truth was provided. "
                    "The model's own answer was used as a proxy — recall scores will be inflated and unreliable."
                )

        except Exception as e:
            st.error(f"RAG'S evaluation failed: {e}")
